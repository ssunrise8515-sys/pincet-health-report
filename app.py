...
app = Flask(__name__)
app.secret_key = os.environ.get("SECRET_KEY", "pincet-secret-2026")
...
.label { font-size: 12px; font-weight: 600; color: #555; margin-bottom: 8px; display: block; }
.pw-input {
  width: 100%;
  padding: 14px 16px;
...
.pw-input:focus { border-color: #1B1B1B; }
.pw-input.error { border-color: #D32F2F; }
.login-btn {
  width: 100%;
  padding: 14px;
  background: #1B1B1B;
...
<div class="card">
  <div class="card-top">
    <div class="logo-mark">핀</div>
    <div class="card-title">핀셋라이프</div>
    <div class="card-sub">건강검진 AI 분석 시스템</div>
  </div>
  <div class="card-body">
    <label class="label">비밀번호를 입력하세요</label>
    <input type="password" class="pw-input" id="pw" placeholder="••••" maxlength="20" onkeydown="if(event.key==='Enter') login()">
    <button class="login-btn" onclick="login()">접속하기</button>
    <div class="error-msg" id="err">비밀번호가 올바르지 않습니다</div>
  </div>
</div>
<script>
...
"""
    ...
    def danger_boxes(metrics):
        html = ""
        for m in metrics:
            html += f"""<div class="alert-num-box">
              <div class="a-val">{m['val']}</div>
              <div class="a-unit">{m['unit']}</div>
              <div class="a-name">{m['name']}</div></div>"""
        return html
...
.alert-bar{background:#D32F2F;padding:4mm 12mm;display:flex;flex-direction:row;align-items:center;gap:10px;flex-shrink:0;}
.alert-num-wrap{display:flex;flex-direction:column;gap:4px;flex-shrink:0;width:52mm;}
.alert-num-box{display:flex;align-items:center;gap:8px;padding:3px 0;border-right:none;border-bottom:0.5px solid rgba(255,255,255,.2);}
.alert-num-box:last-child{border-bottom:none;}
.a-val{font-size:16pt;font-weight:900;color:#fff;line-height:1;min-width:28mm;text-align:right;}
.a-unit{font-size:6.5pt;color:rgba(255,255,255,.8);min-width:12mm;}
.a-name{font-size:6.5pt;font-weight:700;color:#FFCDD2;}
.alert-divider{width:1px;background:rgba(255,255,255,.3);align-self:stretch;margin:0 8px;flex-shrink:0;}
.alert-msg{flex:1;}
...
.nut-grid{display:grid;grid-template-columns:1fr 1fr;gap:7px;flex:1;min-height:0;align-content:stretch;}
.nc{border-radius:6px;padding:11px 12px;display:flex;flex-direction:column;gap:8px;}
...
.nc-badge{font-size:8pt;font-weight:700;padding:3px 9px;border-radius:3px;white-space:nowrap;flex-shrink:0;margin-left:6px;}
...
.nc-danger{font-size:9pt;font-weight:700;color:#B71C1C;}
.nc-ok-msg{font-size:9pt;font-weight:700;color:#1B5E20;}
.nc-wn-msg{font-size:9pt;font-weight:700;color:#BF360C;}
.nc-note{font-size:8.5pt;color:#555;line-height:1.5;}
...
def manifest():
    return send_file(os.path.join(os.path.dirname(__file__), "manifest.json"), mimetype="application/manifest+json")
...
def icon192():
    return send_file(os.path.join(os.path.dirname(__file__), "icon-192.png"), mimetype="image/png")

@app.route("/icon-512.png")
def icon512():
    return send_file(os.path.join(os.path.dirname(__file__), "icon-512.png"), mimetype="image/png")
